...
/**
 * MCC UG seat-matrix PDF parser (v3, debugged for pdf-parse v2+)
 *
 * Purpose:
 * - Parse ONLY the selected MCC UG seat-matrix PDFs
 * - Use the official PDFParse class API from pdf-parse v2+
 * - Extract metadata + text from each PDF
 ...
 *
 * Usage:
 *   node mcc_ug_parse_seat_matrix_pdfs_v3.js --dir=./output/mcc_ug_selected_docs
 */
...
async function main() {
  ...
  const summary = {
    ...
    parser_api: 'PDFParse.getInfo + PDFParse.getText',
  ...
  for (let i = 0; i < seatMatrixItems.length; i += 1) {
    ...
    try {
      ...
      const text = normalizePdfText(textResult && textResult.text ? textResult.text : '');
      ...
      fileMeta.num_pages =
        toNullableInt(infoResult && infoResult.total) ??
        toNullableInt(textResult && textResult.total) ??
        null;
      fileMeta.info = sanitizeJson(infoResult && infoResult.infoData ? infoResult.infoData : null);
