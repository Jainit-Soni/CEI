...
/**
 * MCC UG seat-matrix block parser v3
 *
 * Purpose:
 * - Improve Special Stray MBBS parsing using carried institution context
 * - Fix state carry from header prefixes
 * - Separate address fragments from quota phrases in seat rows
 * - Avoid degrading institution names to generic tokens like 'MEDICAL COLLEGE' or bare pincodes
 *
 * Usage:
 *   node mcc_ug_parse_seat_matrix_blocks_v3.js --dir=./output/mcc_ug_selected_docs
 */
...
const OUTPUT_PATH = path.join(PARSED_DIR, 'mcc_ug_special_stray_block_rows_v3.ndjson');
const META_PATH = path.join(PARSED_DIR, 'mcc_ug_special_stray_block_rows_v3.meta.json');
...
async function main() {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    if (seatRow) {
      if (seatRow.leading_context_raw) {
        const continuation = parseHeaderContinuation(seatRow.leading_context_raw, currentState);
        if (continuation.state_raw) currentState = continuation.state_raw;
        if (continuation.institution_code_raw) currentInstitutionCodeRaw = continuation.institution_code_raw;
        if (continuation.institution_name_guess) currentInstitutionNameGuess = continuation.institution_name_guess;
        if (continuation.header_raw) currentInstitutionHeaderRaw = continuation.header_raw;
      }
    ...
    if (header) {
      ...
      if (header.institution_name_guess) currentInstitutionNameGuess = header.institution_name_guess;
...
function parseSeatRowLine(line, fallbackInstituteCode) {
  ...
  if (quotaHit) {
    ...
    const between = clean(beforeCourse.slice(quotaHit.index + quotaHit.value.length)) || null;
    addressFragmentRaw = between;
  } else {
...
function parseInstitutionHeaderLine(line, currentState) {
  ...
  return {
    ...
    header_raw: cleaned,
    institution_name_guess: guessInstitutionName(headerBody),
  };
}

function parseHeaderContinuation(fragment, currentState) {
  const cleaned = clean(fragment);
  if (!cleaned) return {};
  const prefixedState = detectLeadingState(cleaned);
  const stateRaw = prefixedState ? prefixedState.state_raw : currentState;
  const body = prefixedState ? prefixedState.rest : cleaned;
  return {
    state_raw: stateRaw,
    institution_code_raw: extractInstitutionCode(cleaned),
    header_raw: cleaned,
    institution_name_guess: guessInstitutionName(body),
  };
}
...
function guessInstitutionName(headerRaw) {
  ...
  if (meaningful.length) {
    const best = meaningful.sort((a, b) => b.length - a.length)[0];
    if (/^medical college$/i.test(best) && parts.length > 1) {
  ...
  if (parts.length) {
    const first = parts[0];
    if (/^\d{6}$/.test(first) || /^\(?\d{6}\)?$/.test(first)) return null;
    if (/^[A-Z][a-z]+$/.test(first) && STATES.includes(first.toUpperCase())) return null;
    return first;
  }
  return null;
}
