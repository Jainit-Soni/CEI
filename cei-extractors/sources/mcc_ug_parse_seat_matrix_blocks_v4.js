...
/**
 * MCC UG seat-matrix block parser v4
 *
 * Target:
 * - Special Stray MBBS text dump first
 *
 * Fixes vs v3:
 * - Uses a header buffer across multiple lines
 * - Updates state from both state-only lines and header prefixes
 * - Combines trailing address/code fragments with the buffered institution header
 * - Prevents institution-name collapse to junk like "Pradesh" or generic "MEDICAL COLLEGE"
 * - Separates address fragment from quota phrase in seat rows
 *
 * Usage:
 *   node mcc_ug_parse_seat_matrix_blocks_v4.js --dir=./output/mcc_ug_selected_docs
 */
...
const OUTPUT_PATH = path.join(PARSED_DIR, 'mcc_ug_special_stray_block_rows_v4.ndjson');
const META_PATH = path.join(PARSED_DIR, 'mcc_ug_special_stray_block_rows_v4.meta.json');
...
function mergeHeaderContext(headerBuffer, leadingContextRaw) {
  ...
  if (leadingContextRaw) parts.push(leadingContextRaw);
...
function mergeHeaderParts(parts) {
  return clean(parts.filter(Boolean).map(clean).join(', ')) || null;
}
...
function guessInstitutionName(headerRaw) {
  ...
  if (meaningful.length) {
    const sorted = meaningful.sort((a, b) => scoreInstitutionPhrase(b) - scoreInstitutionPhrase(a));
    return sorted[0];
  }
  ...
  for (const part of parts) {
    ...
    if (/road|nagar|district|dist\.|pin|pincode|near|post|po\b|campus|village|taluka|tehsil|state|city/i.test(part)) continue;
    return part;
  }
  return null;
}

function scoreInstitutionPhrase(text) {
  const t = clean(text);
  let score = 0;
  ...
  if (/^medical college$/i.test(t)) score -= 60;
  if (/^college$/i.test(t)) score -= 80;
  if (/^hospital$/i.test(t)) score -= 80;
  if (/^pradesh$/i.test(t)) score -= 100;
  if (/^faridabad$/i.test(t)) score -= 40;
  if (/^\d+$/.test(t)) score -= 100;
