...
/**
 * CSAB institute profile extractor (text-block parser)
 *
 * Why this version exists:
 * - Many CSAB InstProfile pages expose the seat breakup as a long text block,
 *   not as a clean HTML table that can be parsed via <tr>/<td> structure.
 * - The rendered page text contains a stable heading:
 *     Academic Programwise Seats breakup
 *   followed by line-wise row data.
 *
 * This extractor:
 * - reads csab_institutes.ndjson
 * - visits each InstProfile.aspx page
 * - extracts the seat-breakup section from rendered page text
 * - parses anchor rows + inherited female-only rows + ignores totals-only rows
 * - writes one NDJSON row per seat-pool row
 *
 * Install:
 ...
 *
 * Usage:
 *   node csab_instprofile_extract_text.js --in=./output/csab_institutes.ndjson --out=./output/csab_instprofile_seat_rows.ndjson
 */
...
async function extractProfileSeatRows(page, inst) {
  ...
  const sectionText = await page.evaluate(() => {
    ...
    const marker = 'Academic Programwise Seats breakup';
    const idx = bodyText.indexOf(marker);
    if (idx === -1) return null;

    let tail = bodyText.slice(idx + marker.length);
    const stopMarkers = [
  ...
  if (!sectionText || !sectionText.length) {
    return [];
  }

  const lines = stripNonDataLines(sectionText);
  return parseSeatBreakupLines(lines, inst);
}
...
function stripNonDataLines(lines) {
  return lines.filter((line) => {
    ...
    if (!t) return false;
    if (t === 'Institute Code Program Code Academic Program Name State/All India Seats Seat Pool OPEN OPEN-PwD GEN-EWS GEN-EWS-PwD SC SC-PwD ST ST-PwD OBC-NCL OBC-NCL-PwD Total(includes Female Supernumerary) Program-Total') return false;
    if (t === 'Seat Capacity Female Supernumerary') return false;
    if (t === 'Program-Total') return false;
    return true;
  });
}
...
function parseSeatBreakupLines(lines, inst) {
  ...
  for (const line of lines) {
    ...
    if (/^\(including\s+"?\d+"?\s+Supernumerary\)/i.test(t)) {
      continue;
    }

    if (/^Total Seats\b/i.test(t)) {
      continue;
    }

    const femaleOnly = parseFemaleOnlyLine(t, currentCtx, inst);
    if (femaleOnly) {
      rows.push(femaleOnly);
      continue;
    }

    const anchor = parseAnchorLine(t, inst);
    if (anchor) {
      currentCtx = {
...
function parseAnchorLine(line, inst) {
  const counts = extractTrailingNumbers(line, 13);
  ...
  const stateScope = findBestStateScope(rest);
  if (!stateScope) return null;

  const idx = rest.lastIndexOf(stateScope);
  if (idx === -1) return null;

  const programName = normalizeText(rest.slice(0, idx));
  const afterState = normalizeText(rest.slice(idx + stateScope.length));

  if (!/^Gender-Neutral$/i.test(afterState)) {
    return null;
  }
  ...
  return normalizeSeatRow({
    source: 'csab_instprofile_seat_breakup_text',
...
function parseFemaleOnlyLine(line, currentCtx, inst) {
  ...
  const seatPool = counts.prefix.startsWith('Female-only (including Supernumerary)')
...
function extractTrailingNumbers(line, expectedCount) {
  ...
  return {
    prefix: normalizeText(tokens.join(' ')),
    numbers,
  };
}
...
function findBestStateScope(text) {
  ...
  for (const base of KNOWN_STATE_SCOPES) {
    if (raw.includes(base)) candidates.push(base);
    const otherThan = `Other Than ${base}`;
    if (raw.includes(otherThan)) candidates.push(otherThan);
    const fromState = `From ${base}`;
    if (raw.includes(fromState)) candidates.push(fromState);
  }
