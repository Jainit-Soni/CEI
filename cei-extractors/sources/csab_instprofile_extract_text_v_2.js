...
/**
 * CSAB institute profile extractor (text parser, v2)
 *
 * Why this version exists:
 * - The first text parser was still too brittle.
 * - Real CSAB profile pages do contain the seat-breakup section, but headings,
 *   spacing, and state-scope strings vary.
 *
 * This version hardens:
 * - section detection: regex, not exact substring
 * - section slicing: robust footer stop markers
 * - state-scope parsing: case-insensitive, supports "Other Than ..."
 * - anchor/female row parsing from raw text lines
 *
 * Install:
 ...
 *
 * Usage:
 *   node csab_instprofile_extract_text_v2.js --in=./output/csab_institutes.ndjson --out=./output/csab_instprofile_seat_rows.ndjson
 */
...
async function extractProfileSeatRows(page, inst) {
  ...
  return {
    rows,
    linesPreview: section.linesPreview || [],
  };
}
...
function parseSeatBreakupLines(lines, inst) {
  ...
  for (const line of lines) {
    ...
    const femaleRow = parseFemaleOnlyLine(t, currentCtx, inst);
    if (femaleRow) {
      rows.push(femaleRow);
      continue;
    }

    const anchorRow = parseAnchorLine(t, inst);
    if (anchorRow) {
      currentCtx = {
        institute_code: anchorRow.institute_code,
        institute_name: anchorRow.institute_name,
        program_code: anchorRow.program_code,
        program_name: anchorRow.program_name,
        state_all_india_seats: anchorRow.state_all_india_seats,
        program_total_seat_capacity_raw: anchorRow.program_total_seat_capacity_raw,
        program_total_female_supernumerary_raw: anchorRow.program_total_female_supernumerary_raw,
      };
      rows.push(anchorRow);
    }
  }
...
function parseAnchorLine(line, inst) {
  ...
  const stateIdx = upperRest.lastIndexOf(stateScope);
  if (stateIdx === -1) return null;

  const programName = normalizeText(rest.slice(0, stateIdx));
  const afterState = normalizeText(rest.slice(stateIdx + stateScope.length));
  ...
  const rawCells = [
    ...
    programCode,
    programName,
    toTitleCaseStateScope(stateScope),
    'Gender-Neutral',
    ...nums.map(String),
  ];

  return normalizeSeatRow({
    source: 'csab_instprofile_seat_breakup_text_v2',
