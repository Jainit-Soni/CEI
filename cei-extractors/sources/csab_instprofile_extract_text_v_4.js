...
/**
 * CSAB institute profile extractor (text parser, v4)
 *
 * Fix vs v3:
 * - Do NOT try to match state scopes from a fixed list.
 * - On real CSAB pages, the safest split is:
 *   {inst code} {program code} {program name ending at last ')'} {state scope} Gender-Neutral {11 counts}
 * - This handles scopes like:
 *   DELHI (NCT),CHANDIGARH (UT)
 *   Other Than DELHI (NCT),CHANDIGARH (UT)
 *   PUDUCHERRY (UT),ANDAMAN & NICOBAR ISLANDS (UT)
 ...
 *
 * Usage:
 *   node csab_instprofile_extract_text_v4.js --in=./output/csab_institutes.ndjson --out=./output/csab_instprofile_seat_rows.ndjson
 */
...
function parseAnchorLine(line, inst, totalsPair) {
  ...
  const genderMatch = prefix.match(/^(\d{3,})\s+(\d{3,6})\s+(.*)\s+Gender-Neutral$/i);
  if (!genderMatch) return null;

  const instituteCode = genderMatch[1];
  const programCode = genderMatch[2];
  const beforeSeatPool = normalizeText(genderMatch[3]);

  const split = splitProgramAndStateScope(beforeSeatPool);
  ...
  return normalizeSeatRow({
    source: 'csab_instprofile_seat_breakup_text_v4',
...
function splitProgramAndStateScope(text) {
  ...
  const programName = normalizeText(raw.slice(0, lastParen + 1));
  const stateScope = normalizeText(raw.slice(lastParen + 1));

  if (!programName || !stateScope) return null;

  return { programName, stateScope };
}
