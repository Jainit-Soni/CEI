...
/**
 * CSAB institute profile extractor (text parser, v5)
 *
 * Fix vs v4:
 * - Anchor rows are parsed with a program-name regex, not by splitting on the last ')'.
 * - This handles complex quota scopes like:
 ...
 *
 * Pattern handled:
 *   {inst code} {program code} {program name ending with degree block} {state scope} Gender-Neutral {11 counts}
 *   {seat capacity} {female supernumerary}
 *   Female-only ... {11 counts}
 *
 * Install:
 ...
 *
 * Usage:
 *   node csab_instprofile_extract_text_v5.js --in=./output/csab_institutes.ndjson --out=./output/csab_instprofile_seat_rows.ndjson
 */
...
function parseAnchorLine(line, inst, totalsPair) {
  ...
  const m = prefix.match(/^(\d{3,})\s+(\d{3,6})\s+(.+?\(\d+\s+Years?,\s+[^)]+\))\s+(.+)\s+Gender-Neutral$/i);
  if (!m) return null;

  const instituteCode = m[1];
  const programCode = m[2];
  const programName = normalizeText(m[3]);
  const stateScope = normalizeText(m[4]);
  ...
  return normalizeSeatRow({
    source: 'csab_instprofile_seat_breakup_text_v5',
