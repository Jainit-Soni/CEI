...
# ========= APIFY PAGE FUNCTION =========
# Pure DOM polling. No page.waitForSelector, no Puppeteer-only calls.
PAGE_FUNCTION = r"""
async function pageFunction(context) {
    const { request, log } = context;
    const selector = 'a[href*="/Download/Notice/"]';

    // Wait up to 20s for notice links to appear.
    ...
    const rows = anchors.map((a, idx) => {
        ...
        // Common pattern: "12 Some notice title..."
...
def main():
    ...
    if not token:
        ...
        print('Set it in CMD first, for example:')
        print('set APIFY_TOKEN=YOUR_REAL_TOKEN')
        sys.exit(1)
    ...
    # If scraper failed and stored #error items, surface that immediately.
    error_items = [x for x in items if isinstance(x, dict) and x.get("#error")]
    if error_items:
        save_json(JSON_FILE, items)
        print(f"Run finished, but dataset contains {len(error_items)} error item(s).")
        print(f"Saved raw output to: {JSON_FILE.resolve()}")
        print("First error:")
    ...
    for item in normalized:
        ...
        file_name = f"{int(idx):03d} - {title} - {original_name}"
